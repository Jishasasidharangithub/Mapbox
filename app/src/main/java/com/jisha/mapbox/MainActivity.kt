...
import com.jisha.mapbox.databinding.DialogProgressBinding
import com.jisha.mapbox.ConstantsUtils.locationDestination
import com.jisha.mapbox.ConstantsUtils.locationStart
import com.jisha.mapbox.LocationHandler.isGpsEnabled
import com.jisha.mapbox.LocationHandler.isLocationApproved
import com.jisha.mapbox.ScreenUtils.getScreenHeight
import com.jisha.mapbox.ScreenUtils.getScreenWidth
...
class MainActivity : AppCompatActivity() {
    ...
    private fun initProgressDialog() {
        ...
        dialogBinding.progressCard.layoutParams.width =
            (getScreenWidth() * 0.90).toInt()
        dialogBinding.progressCard.layoutParams.height =
            (getScreenHeight() * 0.10).toInt()
    }
    ...
    private fun findAddress(mLoc: MapLocation) {
        ...
        CoroutineScope(Dispatchers.Main + handler).launch {
            async(Dispatchers.IO+handler) {
                ...
                addresses = geocoder.getFromLocation(mLoc.placeLatitude, mLoc.placeLongitude, 1) as List<Address>
            }.await()
            runOnUiThread {
                progressDialog?.dismiss()
            }
        ...
        CoroutineScope(Dispatchers.Main + handler).launch {
            try {
                ...
                if (myCurrentAddress.equals(startAddress)){
                    ...
                    async(Dispatchers.IO+handler) {
                        val geocoder = Geocoder(this@MainActivity, Locale.getDefault())
                        endResponses = geocoder.getFromLocationName(endAddress, 1) as List<Address>
                    ...
                    async(Dispatchers.IO+handler) {
                        val geocoder = Geocoder(this@MainActivity, Locale.getDefault())
                        startResponses = geocoder.getFromLocationName(startAddress, 1) as List<Address>
